...
static int unpack_file(FILE *pack) {
        ...
        if (fd >= 0) {
                ...
                if (st.st_ino != (uint64_t) inode) {
                        log_error("inode of %s was changed. ignore", fn);
                        close_nointr_nofail(fd);
                        fd = -1;
                }
        }
        ...
        for (;;) {
                ...
                log_debug("%s(%d): page %u to %u", fn, fd, b, c);

                any = true;

                if (fd >= 0)
                        if (posix_fadvise(fd, b * page_size(), (c - b) * page_size(), POSIX_FADV_WILLNEED) < 0) {
                                log_warning("posix_fadvise() failed: %s", strerror(errno));
                                goto finish;
                        }
        }
        ...
        if (!any && fd >= 0) {
                ...
                log_debug("%s(%d): 0 to %u", fn, fd, st.st_size);
...
static int replay(const char *root) {
        ...
        if (access(READAHEAD_DIR "/noreplay", F_OK) >= 0) {
