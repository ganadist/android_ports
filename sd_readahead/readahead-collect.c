...
static int pack_file(FILE *pack, const char *fn, bool on_btrfs) {
        ...
        if (fd < 0) {
                ...
                log_warning("open(%s) failed: %s", fn, strerror(errno));
                r = -errno;
                goto finish;
        }
...
static int collect(const char *root) {
        ...
        if (!strncmp(root, ANDROID_SYSTEM, strlen(ANDROID_SYSTEM))) {
            pack_fn = strdup(PACK_FILE);
        ...
        if (access(READAHEAD_DIR "/cancel", F_OK) >= 0) {
                log_debug("Collection canceled");
                r = -ECANCELED;
                goto finish;
        }

        if (access(READAHEAD_DIR "/done", F_OK) >= 0) {
        ...
        if (!strncmp(root, ANDROID_SYSTEM, strlen(ANDROID_SYSTEM))) {
            pack_fn_new = strdup(PACK_FILE_NEW);
